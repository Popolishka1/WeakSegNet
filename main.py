...
def parse_args():
    parser = argparse.ArgumentParser(description="Weak supervision segmentation experiment.")
...
def main():
    ...
    baseline_model = select_model(config["baseline_model"])
    ...
    if config["train"]:
        baseline_model = train_model_segmentation(baseline_model, train_loader, val_loader, config, device)
